...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if ($id_categorie > 0) {
        $insertProductQuery = "INSERT INTO produit (stock, id_categorie, nam, prix, age, img, description, date_creation,etat) VALUES (?, ?, ?, ?, ?, ?, ?, ?,?)";
        $stmt = $con->prepare($insertProductQuery);
        $stmt->bind_param("iississsi", $stockSend, $id_categorie, $nomSend, $prixSend, $ageSend, $imageNames, $descSend, $dateSend, $etat);
        $stmt->execute();
